...
const CreateTaskForm = (props) => {
  ...
  useEffect(()=> {
    ...
    if(taskData.taskDate.length === 0){
      ...
      isFormValid = false;
    } else if(new Date(taskData.taskDate).setHours(0, 0, 0, 0) < new Date().setHours(0, 0, 0, 0)) {
      dateErrorMessage = "The date should not be in the past";
      isFormValid = false;
      console.log(new Date(taskData.taskDate), new Date());
    }

    if(taskData.taskDescription.length === 0){
      descriptionErrorMessage = "This field is required!";
      isFormValid = false;
    } else if(taskData.taskDescription.length < 4 && 
  ...
  return (
    <div className="create-task-form">
      ...
      <form onSubmit={handleSubmitForm}>
        ...
        <Button textBtn="Create task" type="submit" isValid={formErrors.isValid} />
      </form>
    </div>
  );
};
